					...
					if (connections.has(agentId)) {
						...
						cSocket.connect(port, ip, function() {
							circuitSocket.on('data', function(data) {
								...
								} else if (cellType === OPEN_FAILED) {
									
								} else if (cellType === CREATE_FAILED) {

								} else {
									// just swap the circuit and send back to inSocket
								}
...
*/
									...
									console.log('create fail sent from inner socket');
									var innerCircuit = data.readUInt16BE(0);

									routingData.set(circuitSocket, new Map());
									routingData.get(circuitSocket).set(innerCircuit, {'socket' : socket, 'circuit' : circuit});

									circuitSocket.write(createFail(innerCircuit));
									return;
								} else {
									console.log('?????');
								}
...
function startup(entries) {
	...
	var circuitSocket = new net.Socket();
	circuitSocket.connect(entry.entryPort, entry.entryIp, function() {
