...
var server = net.createServer(function(socket) {
	...
	socket.on('data', function(data) {
		...
		var rSocket = new net.Socket();	

		rSocket.on('error', function(err) {
			console.log('ERROR forwarding request');
			socket.end();
		});

		var responseSocket = rSocket;

		// The port sharing problem was most likely a scoping issue with respect to the responseSocket variable
		rSocket.connect(port, host, function() {
			...
			responseSocket.on('data', function(responseData) {
				...
				if (responseHeaderIndex < 0) {
					if (buffers.get(responseSocket)) {
						...
						buffers.set(responseSocket, Buffer.concat([buf, responseData]));
					} else {
						buffers.set(responseSocket, responseData);
					}
					return;
				}
